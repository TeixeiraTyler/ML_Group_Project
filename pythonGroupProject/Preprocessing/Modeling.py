...
class Modeling:
	...
	def go(self, combinedData, totalCols, test_ID, colsP, RFEcv, XGBestCols):
		train = combinedData.loc[combinedData.SalePrice>0 , list(totalCols)].reset_index(drop=True, inplace=False)
		y_train = combinedData.SalePrice[combinedData.SalePrice>0].reset_index(drop=True, inplace=False)
		test = combinedData.loc[combinedData.SalePrice==0 , list(totalCols)].reset_index(drop=True, inplace=False)
