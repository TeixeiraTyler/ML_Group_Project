...
class SelectFeatures:
	...
	def go(self, combinedData, cols, polynomialColumns):
		self.combinedData = combinedData.loc[(combinedData.SalePrice > 0), cols].reset_index(drop = True, inplace = False)
		y_train = combinedData.SalePrice[combinedData.SalePrice > 0].reset_index(drop = True, inplace = False)

		robustScaler = RobustScaler();
		robustScalerDataFrame = pd.DataFrame(robustScaler.fit_transform(self.combinedData[cols]), columns = cols)
		...
		return self.combinedData, unionSetOfBestColumnsList, recursiveFeatureSelectedColumns, XGBestColumns
